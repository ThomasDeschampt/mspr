...
router.get("/afficher", async (req, res) => {
  ...
  try {
    ...
    console.error("Erreur lors de la récupération de limage:", erreur.message);
    res.status(500).json({
      message: "Erreur lors de la récupération de limage:",
...
router.post("/ajouter", async (req, res) => {
  ...
  try {
    await ajouterImage(url_img, id_utl, id_utl_1, id_plt);
  } catch (erreur) {
