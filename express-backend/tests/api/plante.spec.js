...
describe('Test des fonctions liées aux plantes', () => {
  ...
  describe('Test de afficherPlante', () => {
    ...
    it('Devrait afficher une erreur en cas de problème', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Erreur lors de la récupération de la plante:', erreur.message);
    });
  });
  ...
  describe('Test de afficherPlanteGardees', () => {
    ...
    it('Devrait afficher une erreur en cas de problème', async () => {
      const erreur = new Error('Erreur lors de la récupération des plantes gardees');
      ...
      expect(console.error).toHaveBeenCalledWith('Erreur lors de la récupération des plantes gardees:', erreur.message);
    });
  });
  ...
  describe('Test de afficherPlanteFaitesGardees', () => {
    ...
    it('Devrait afficher une erreur en cas de problème', async () => {
      const erreur = new Error('Erreur lors de la récupération des plantes faites gardees');
      Plante.findAll.mockRejectedValue(erreur);
      ...
      expect(console.error).toHaveBeenCalledWith('Erreur lors de la récupération des plantes faites gardees:', erreur.message);
    });
  });

  describe('Test de recupererlocalisation', () => {
    it('Devrait récupérer les localisations des plantes sans gardien', async () => {
      const plantesSansGardienMock = [{ localisation: 'localisation1' }, { localisation: 'localisation2' }]; 
    ...
    it('Devrait afficher une erreur en cas de problème', async () => {
      const erreur = new Error('Erreur lors de la récupération de la localisation de la plante');
      Plante.findAll.mockRejectedValue(erreur);
      ...
      expect(console.error).toHaveBeenCalledWith('Erreur lors de la récupération de la localisation de la plante:', erreur.message);
    });
  });

  describe('Test de ajouterPlante', () => {
    it('Devrait ajouter une nouvelle plante', async () => {
        const nouvellePlanteMock = { id_plt: 5, esp_plt: 'Espèce', dsc_plt: 'Description', nom_plt: 'Nom', adr_plt: 'Adresse', dat_deb_plt: '2024-02-27', dat_fin_plt: '2024-03-27', id_utl: 1, id_utl_1: 3 };
        Plante.create.mockResolvedValue(nouvellePlanteMock);
      
        console.log = jest.fn();
      
        await ajouterPlante('Espèce', 'Description', 'Nom', 'Adresse', '2024-02-27', '2024-03-27', 1);
      
        expect(Plante.create).toHaveBeenCalledWith({ esp_plt: 'Espèce', dsc_plt: 'Description', nom_plt: 'Nom', adr_plt: 'Adresse', dat_deb_plt: '2024-02-27', dat_fin_plt: '2024-03-27', id_utl: 1, id_utl_1: 3 });
        expect(console.log).toHaveBeenCalledWith('Nouvelle plante ajoutée:', nouvellePlanteMock);
      });
    ...
    it('Devrait afficher une erreur en cas de problème', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Erreur lors de l\'ajout de la plante:', erreur.message);
    });
  });
  ...
  describe('Test de ajouterGardienPlante', () => {
    it('Devrait ajouter un gardien à la plante spécifiée', async () => {
      ...
      expect(Plante.update).toHaveBeenCalledWith({ id_utl_1: 1 }, { where: { id_plt: 5 } });
    ...
    it('Devrait afficher une erreur en cas de problème', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Erreur lors de l\'ajout du gardien à la plante:', erreur.message);
    });
  });
});
