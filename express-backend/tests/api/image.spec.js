const { afficherImage, afficherImages, ajouterImage } = require('../../api/Image');
...
describe('Test des fonctions d\'image', () => {
  ...
  describe('Test de afficherImage', () => {
    ...
    it('Devrait afficher une erreur en cas de problème', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Erreur lors de la récupération de l\'image:', erreur.message);
    });
  });
  ...
  describe('Test de afficherImages', () => {
    it('Devrait retourner les images du gardiennage', async () => {
      ...
      expect(Image.findAll).toHaveBeenCalledWith({ where: { id_utl: 1, id_utl_1: 2, id_plt: 3 } });
    });

    it('Devrait afficher une erreur en cas de problème', async () => {
      const erreur = new Error('Erreur lors de la récupération des images du gardiennage');
      ...
      expect(console.error).toHaveBeenCalledWith('Erreur lors de la récupération des images du gardiennage:', erreur.message);
    });
  });

  describe('Test de ajouterImage', () => {
    it('Devrait ajouter une nouvelle image', async () => {
        const nouvelleImageMock = { id_img: 1, url_img: 'url', id_utl: 1, id_utl_1: 2, id_plt: 3 };
        Image.create.mockResolvedValue(nouvelleImageMock);
      
        await ajouterImage('url', 1, 2, 3);
      
        expect(Image.create).toHaveBeenCalledWith({ url_img: 'url', id_utl: 1, id_utl_1: 2, id_plt: 3 });
      });
    ...
    it('Devrait afficher une erreur en cas de problème', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Erreur lors de l\'ajout de l\'image:', erreur.message);
    });
  });
});
